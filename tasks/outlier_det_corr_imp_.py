#TEST OUTLIER DETECTION, CORRECTION E DATA IMPUTATION
...
def save_results(od_method, oc_method, imp_method, percentage, r2):
    path = "../Risultati/Air_Quality/rf_regression_mix_outliers_none_1.csv"
...
for od_method in outlier_detection_methods:
    for oc_method in outlier_correction_methods:
        for imp_method in imputation_methods:
            ...
            c_outlier = 0
            outliers = []
            pippo = [] #maschera utilizzata per distinguere gli outlier dai missing values
            actual_values = []
            predicted_values = []
            ...
            for message in consumer:
                ...
                if full_window_flag:
                    if ml_method == 'regression':
                        actual_values, predicted_values = regression(df, slide, columns, actual_values,
                                                                     predicted_values, target_cols,
                                                                     list_forest_importance)
                    elif ml_method == 'classification':
                        actual_values, predicted_values = classification(df, slide, columns, actual_values,
                                                                         predicted_values,
                                                                         target_cols)
