...
def app():
    ...
    if df is not None:
        ...
        if right_df == "No":
            ...
            profilingReportBox = st.checkbox("Select to generate the profiling report and start analyzing your dataset:", value=False, key=11)
            if profilingReportBox == True:
                try:
                    ...
                    with open("/Users/matteopancini/Desktop/PII/APP/newProfile.json", 'r') as f:
                        report = json.load(f)
            ...
            if profileGenerated == True:
                ...
                if showActions == True:
                    ...
                    dataActions = st.sidebar.selectbox("Select a data operation:", ("", "Data Profiling", "Data Wrangling"), index=0)
                    ...
                    if(dataActions == "Data Profiling"):
                        ...
                        singleColumnAnalysisOptions = st.sidebar.radio("Single Column Analysis", ("Cardinalities", "Value Distribution", "Data Types"), key=101)
                        if singleColumnAnalysisOptions == "Cardinalities":
                            ...
                            with col3:
                                st.metric(label="Completeness (number of none-null values)", value=total_completeness, delta=percent_completeness)
                            with col4:
                                st.metric(label="Uniqueness (number of distinct values)", value=uniqueValues, delta=percent_uniqueness)

                        elif singleColumnAnalysisOptions == "Value Distribution":
                            st.subheader("Value Distribution")
                            st.markdown("We are filtering the columns of your database including just numeric values, in order to analyze them!")
                            dfCol = []
                            ...
                            if len(warningToPrint) != 0:
                                for warningMex in warningToPrint:
                                    st.info("The column " + columnChosen + " chosen presents the following warning: " + str(warningMex))
                            else:
                            ...
                            st.markdown("The column " + columnChosen + " chosen has type: " + str(df[columnChosen].dtype))
                    ...
                    if(dataActions == "Data Wrangling"):
                        ...
                        firstNewRows = st.checkbox("Select to show a preview of your wrangling dataset:", value=False, key=13)
                        ...
                        wranglingOperations = ["Column Splitting", "Handling Missing Values", "Handling Dates", "Column Merging", "Column Renaming"]
                        activeOperations = []
                        for op in wranglingOperations:
                            activeOperations.append(st.sidebar.checkbox(op, value=False, key=14))
                        ...
                        for x in range(len(activeOperations)):
                            if(activeOperations[x] == True):
                                ...
                                if x == 0:
                                    ...
                                    st.markdown("This wrangling operation let you split a column composed of multiple values.")
                                    st.info("👀Please select the column first, then the delimiter and then call your new columns whatever you want!")
                                    dfCol = []
                                    for col in wrangler_df.columns:
                                    ...
                                    if(len(dfCol) > 0):
                                        ...
                                        splittingDelimiter = st.selectbox("Select the delimiter of " + columnChosen, (" ", ","), 0)
                                        if splittingDelimiter == " ":
                                            if " " not in str(wrangler_df[columnChosen][0]):
                                                st.error("Sorry, there are no tuples in column " + columnChosen + " that agree with your pattern.\nPlease update column or delimiter!")
                                            else:
                                                st.success("Possibile wrangling!")

                                                firstColumn = st.text_input("Insert the name of the new column before the splitter: ")
                                                secondColumn = st.text_input("Insert the name of the new column after the splitter: ")
                                                split1 = wrangler_df[columnChosen].str.split(expand=True)

                                                applySplit = st.checkbox("Click here to apply", value=False, key=15)

                                                if applySplit == True:
                                                    wrangler_df[[firstColumn, secondColumn]] = wrangler_df[columnChosen].str.split(splittingDelimiter, n=1, expand=True)
                                                    st.success("🎉Congrats!\nYou splitted column " + columnChosen + " into the two new columns: " + firstColumn + ", " + secondColumn)

                                                    deleteSplittedColumn = st.radio("Do you want to delete your splitted column " + columnChosen + "?", ("Yes", "No"), 1, key=105)
                                                    ...
                                                    showSplit = st.checkbox("Click here to show the new wrangled-dataset", value=False, key=16)
                                                    if showSplit:
                                                        st.write(wrangler_df.head())
                                        ...
                                        if splittingDelimiter == ",":
                                            if "," not in str(wrangler_df[columnChosen][0]):
                                                ...
                                                if applySplit == True:
                                                    ...
                                                    deleteSplittedColumn = st.radio("Do you want to delete your splitted column " + columnChosen + "?", ("Yes", "No"), 1, key=106)
                                                    if deleteSplittedColumn == "Yes":
                                                        wrangler_df = wrangler_df.drop(columnChosen, axis=1)
                                                        st.info("Column " + columnChosen + " has been removed!")
                                                    else:
                                                    ...
                                                    wrangler_df_csv = convert_df(wrangler_df)
                                                    showSplit = st.checkbox("Click here to show your splitted dataset", value=False, key=18)
                                                    if showSplit:
                                                        st.write(wrangler_df.head())

                                                    operationApllied = True
                                    else:
                                        st.error("Sorry, there are no tuples in your dataset that agree with your pattern. Please select another wrangling operation!")
                                ...
                                if x == 1:
                                    ...
                                    st.markdown("This wrangling operation let you handle in different modes the missing values of your dataset.")
                                    writeColumns()
                                    ...
                                    st.markdown("Your dataset has a total percentage of missing values of: " + total_percent_missing)
                                    ...
                                    if(n_cells_missing == 0):
                                        ...
                                        st.markdown("You can handle missing values in 2 possible ways: by dropping or by filling!")
                                        dropOrReplace = st.selectbox("Select your handling mode:", ("", "Drop Data", "Fill Data"), index=0)

                                        if(dropOrReplace == "Drop Data"):
                                            st.markdown("The dropping data mode is composed of other 2 options: full-drop mode and column-based-drop mode!")
                                            st.markdown("Choose the mode which best suits your necessities.")
                                            dropOption = st.selectbox("Check to show drop options:", ("", "Full-Drop Mode", "Column-Based-Drop Mode"), index=0)

                                            if(dropOption == "Full-Drop Mode"):
                                                wrangler_df = wrangler_df.dropna()
                                                st.markdown("Force the checkbox below if you want to delete every possible row that contains one or more missing values.")
                                            ...
                                            if(dropOption == "Column-Based-Drop Mode"):
                                                st.markdown("In this mode you can handle all the missing values for every colum that presents missing values.")
                                                ...
                                                for z in range(len(dfColToDrop)):
                                                    ...
                                                    if dfColToDrop[z] == True:
                                                        ...
                                                        realPercentCellMissing = "{:.2f}".format(percentCellMissing) + "%"
                                                        st.markdown("Column " + dfCol[z] + " has a total of " + str(columnCellMissing) + " missing values, which is " + str(realPercentCellMissing) + ".")
                                                ...
                                                if showStats == True:
                                                    ...
                                                    with col1:
                                                        st.metric(
                                                            label="Before wrangling missing percentage", value=total_percent_missing)
                                                    with col2:
                                                        ...
                                                        afterTotalPercentMissing = "{:.2f}".format(afterMissingPercent) + "%"

                                                        st.metric(label="After wrangling missing percentage", value=afterTotalPercentMissing)


                                                    missingWrangle = st.checkbox("Check to wrangle", value=False, key=21)
                                                    ...
                                                    if missingWrangle == True:
                                                        ...
                                                        newDF1 = st.checkbox("Select to show side by side comparison of your new-born wrangled dataset and the old one:", value=False, key=22)
                                        ...
                                        if(dropOrReplace == "Fill Data"):
                                            ...
                                            st.markdown("Below you can find some possibilities of filling the missing values of your columns.")
                                            ...
                                            for y in range(len(dfColToFill)):
                                                ...
                                                if dfColToFill[y] == True:
                                                    ...
                                                    if df[dfCol[y]].dtype == "object":
                                                        fillingOpObj = st.radio("Filling options for " + dfCol[y] + ":", ("", "Following Value", "Previous Value", "Mode", "Custom Value"), index=0, key=107)

                                                        if fillingOpObj == "Following Value":
                                                            if report["variables"][dfCol[y]]["n_missing"] == 0:
                                                                st.success("Column " + dfCol[y] + " has no missing values!")
                                                            else:

                                                                st.info("✔️ Filled all missing values of column " + dfCol[y] + " with its following value")
                                                                wrangler_df[dfCol[y]].fillna(method="bfill", inplace=True)
                                                        ...
                                                        if fillingOpObj == "Previous Value":
                                                            if report["variables"][dfCol[y]]["n_missing"] == 0:
                                                                ...
                                                                st.info("✔️ Filled all missing values of column " + dfCol[y] + " with its previous value")
                                                                wrangler_df[dfCol[y]].fillna(method="ffill", inplace=True)
                                                        ...
                                                        if fillingOpObj == "Mode":
                                                            if report["variables"][dfCol[y]]["n_missing"] == 0:
                                                                ...
                                                                st.info("✔️ Filled all missing values of column " + dfCol[x] + " with its mode")
                                                                    
                                                                wrangler_df[dfCol[y]].fillna(wrangler_df[dfCol[y]].mode()[0], inplace=True)
                                                        ...
                                                        if fillingOpObj == "Custom Value":
                                                            if report["variables"][dfCol[y]]["n_missing"] == 0:
                                                                ...
                                                                if len(customValue) != 0:
                                                                    wrangler_df[dfCol[y]].replace([np.nan], customValue, inplace=True)
                                                                    st.info("✔️ Filled all missing values of column " + dfCol[y] + " with value " + customValue)
                                                        ...
                                                        fillingOpNum = st.radio("Filling options for " + dfCol[y] + ":", ("", "Max", "Avg", "0", "Mode"), index=0, key=108)
                                                        ...
                                                        if fillingOpNum == "Max":
                                                            if report["variables"][dfCol[y]]["n_missing"] == 0:
                                                                ...
                                                                st.info("✔️ Filled all missing values of column " + dfCol[y] + " with its max value: " + maxValue2)
                                                                wrangler_df[dfCol[y]].replace([np.nan], maxValue, inplace=True)
                                                        ...
                                                        if fillingOpNum == "Avg":
                                                            if report["variables"][dfCol[y]]["n_missing"] == 0:
                                                                ...
                                                                st.info("✔️ Filled all missing values of column " + dfCol[y] + " with its average value: " + avgValue2)
                                                                wrangler_df[dfCol[y]].replace([np.nan], avgValue, inplace=True)
                                                        ...
                                                        if fillingOpNum == "0":
                                                            if report["variables"][dfCol[y]]["n_missing"] == 0:
                                                                ...
                                                                st.info("✔️ Filled all missing values of column " + dfCol[y] + " with value 0")
                                            ...
                                            if oneFillingDone == True:
                                                newDFFilled = st.checkbox("Check to show side by side comparison of your new-born wrangled dataset and the old one:", value=False, key=24)
                                                if newDFFilled == True:
                                                    col1, col2 = st.columns(2)
                                ...
                                if x == 2:
                                    ...
                                    st.info("⚠️Remember that this operation, in case your column isn't an actual date, will catch an error! Be Careful!")
                                    ...
                                    if df[columnHandlingChosen].dtype == "float64":
                                        st.markdown("Column \"" + columnHandlingChosen + "\" has type: " + beforeWranglingDT)

                                        if df[columnHandlingChosen].dtype == "datetime64[ns]":
                                                st.success("Column \"" + columnHandlingChosen + "\" has already the right datetime type!")
                                        else:

                                            st.error("Invalid type of column \"" + columnHandlingChosen + "\".\nPlease change column!")

                                    elif df[columnHandlingChosen].dtype == "object":
                                        st.success("🎉Possible wrangling!")
                                        dateWrangle = st.checkbox("Check to switch type of column " + columnHandlingChosen, value=False, key=25)

                                        if dateWrangle:
                                            try:
                                                wrangler_df[columnHandlingChosen] = pd.to_datetime(wrangler_df[columnHandlingChosen], errors = 'coerce')
                                                ...
                                                if showWrangleDate == True:
                                                    ...
                                                    with col1:
                                                        st.metric("Before wrangling column " + columnHandlingChosen + " data type:", beforeWranglingDT)
                                                    with col2:
                                                        st.metric("After wrangling column " + columnHandlingChosen + " data type:", str(wrangler_df[columnHandlingChosen].dtype))
                                ...
                                if x == 3:
                                    ...
                                    st.info("👀Remember: the first column chosen is the root-column of the mering operation!")
                                    ...
                                    for col in wrangler_df.columns:
                                        if(col != firstMergingColumn):
                                            ...
                                            dfColToMerge.append(st.checkbox(col, value=False, key=26))
                                        ...
                                        if mergingPossibility == True:
                                            ...
                                            for h in range(len(dfColToMerge)):
                                                ...
                                                if dfColToMerge[h] == True:
                                                    ...
                                                    wrangler_df[firstMergingColumn] = wrangler_df[firstMergingColumn].map(str) + "," + wrangler_df[dfColMerge[h]].map(str)
                                                    wrangler_df_csv = convert_df(wrangler_df)


                                            if operationApllied:
                                                st.info("💡Now that you merged some columns you can use the \"Column renaiming\" operation for column " + firstMergingColumn)


                                            showMerge = st.checkbox("Select to show your merged dataset", value=False, key=27)
                                ...
                                if x == 4:
                                    ...
                                    for columnIn in wrangler_df.columns:
                                        ...
                                        dfColToRename.append(st.checkbox(columnIn, value=False, key=28))
                                    ...
                                    if renaming == True:
                                        for q in range(len(dfColToRename)):
                                            ...
                                            if dfColToRename[q] == True:
                                                newName = st.text_input("Please insert a new name for column " + str(dfColRename[q]) + ":")
                                                if len(newName) > 0:
                                                    ...
                                                    wrangler_df.to_csv(r'/Users/matteopancini/Desktop/PII/APP/NewDF.csv', index=False)
                                        ...
                                        if oneRename == True:
                                            showRenaming = st.checkbox("Select to show your column-renamed dataset", value=False, key=29)
                        ...
                        if operationApllied == True:
                            ...
                            showDlButton = st.sidebar.radio("❓Have you finished your operations?", ("No", "Yes"), index=0, key=111)
